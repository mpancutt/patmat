...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      def accumulate(charsToProcess: List[Char], charMap: List[(Char, Int)]): List[(Char, Int)] = {
        charsToProcess match {
          case char :: tail => accumulate(
            tail, charMap.map { case (c, number) => (c, (if (c == char) number + 1 else number)) }
          )
          case Nil => charMap
        }
      }

      accumulate(chars, chars.distinct.map(char => (char, 0)))
    }
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
      case Nil => Nil
      case (a, bits) :: rest => if (a == char) bits else codeBits(rest)(char)
    }
   ...
   */
    def convert(tree: CodeTree): CodeTable = {
      ...
      def encode(char: Char)(t: CodeTree): List[Bit] = t match {
        case f: Fork => if(contains(char, f.left)) 0 :: encode(char)(f.left) else 1 :: encode(char)(f.right)
        case l: Leaf => Nil
      }

      def table(chars: List[Char]): CodeTable =
        chars match {
          case char :: rest => (char -> encode(char)(tree)) :: table(rest)
          case Nil => Nil
        }
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
      def merge(bit: Bit)(table: CodeTable): CodeTable = table.map { case (char, bits) => (char, bit :: bits) }
      merge(0)(a) ::: merge(1)(b)
    }
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      ...
      text match {
        case char :: rest => table(char) ::: quickEncode(tree)(rest)
        case Nil => Nil
      }
    }
  }
